...
HammDroid is a local-first job-search agent. The agent should automate repetitive discovery and tracking work while keeping sensitive or consequential actions behind a human approval step.
...
The spreadsheet acts as the system's lightweight state layer rather than relying on browser automation for every update.
